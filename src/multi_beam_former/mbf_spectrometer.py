...
class MBFSpectrometer(SpectraAnimator):
    ...
    def __init__(self, calanfpga):
        ...
        self.write_phasor_reg_list(16*[1-2**-bin_pt], range(16), self.settings.cal_phase_info)

    def write_phasor_reg(self, phasor, addrs, phase_bank_info):
        """
        ...
        self.fpga.set_reg(phase_bank_info['phasor_regs'][0], phasor_re)
        self.fpga.set_reg(phase_bank_info['phasor_regs'][1], phasor_im)

        # 2. write address register(s)
        if isinstance(phase_bank_info['addr_regs'], str): # case one register
            self.fpga.set_reg(phase_bank_info['addr_regs'], addrs)
        
        else: # case multiple registers
            for addr_reg, addr in zip(self.phase_bank_info['addr_regs'], addr):
                self.fpga.set_reg(addr_reg, addr)
                
        # 3. posedge in we register
        self.fpga.set_reg(phase_bank_info['we_reg'], 1)
        self.fpga.set_reg(phase_bank_info['we_reg'], 0)

    def write_phasor_reg_list(self, phasor_list, addr_list, phase_bank_info):
        """
        ...
        for phasor, addr in zip(phasor_list, addr_list):
            self.write_phasor_reg(phasor, addr, phase_bank_info)
